...
export class WaveTune {
  ...
  // Drum specific
  private kick: { triggerAttackRelease: (note: string, duration: string, time: number) => void } | null = null;
  private snare: { triggerAttackRelease: (duration: string, time: number) => void } | null = null;
    ...
    if (this.instruments.drums?.on) {
      ...
      this.kick = {
        triggerAttackRelease: (note: string, duration: string, time: number) => {
          if (this.samplers.drums?.loaded) {
            this.samplers.drums.triggerAttackRelease('C1', duration, time);
          }
        }
      };
      ...
      this.snare = {
        triggerAttackRelease: (duration: string, time: number) => {
          if (this.samplers.drums?.loaded) {
            this.samplers.drums.triggerAttackRelease('D1', duration, time);
          }
        }
      };
    }
  ...
  private getCurrentPattern(instrumentName: string): (string | '-')[] {
    ...
    const instrument = this.instruments[instrumentName as keyof Instruments];
  ...
  private advanceSequence(instrumentName: string): void {
    ...
    if (state.repeatCount >= currentSeq.repeat) {
      state.repeatCount = 0;
      state.sequenceIndex = (state.sequenceIndex + 1) % instrument.sequence.length;
      ...
      if (instrumentName === 'guitar' && nextSeq.distortion !== undefined) {
        this.guitar = nextSeq.distortion ? this.guitarDistorted : this.guitarClean;
        console.log(`Guitar: Switching to ${nextSeq.distortion ? 'distorted' : 'clean'}`);
      }
      
      // Check if guitar sequence is complete
      if (instrumentName === 'guitar' && state.sequenceIndex === 0 && !this.guitarSequenceComplete) {
      ...
      this.loops.bass = new Tone.Loop(time => {
        ...
        if (note !== '-' && note) {
          if (CHORD_DEFINITIONS[note]) {
            const chordNotes = CHORD_DEFINITIONS[note];
            chordNotes.forEach(chordNote => {
              this.samplers.bass?.triggerAttackRelease(chordNote, '8n', time);
            });
          } else {
  ...
  async start(enableMic = false): Promise<void> {
    ...
    if (enableMic && this.microphone && this.micMeter && this.micGain) {
      try {
        ...
        alert('Could not access microphone. Recording will continue without vocals.');
      }
    }
    ...
    if (this.instruments.guitar?.on) {
      ...
      if (firstGuitarSeq?.distortion !== undefined) {
        this.guitar = firstGuitarSeq.distortion ? this.guitarDistorted : this.guitarClean;
        console.log(`Guitar: Starting with ${firstGuitarSeq.distortion ? 'distorted' : 'clean'}`);
      }
    }
  ...
  stop(): void {
    ...
    if (this.recorder?.state === 'started' && !this.guitarSequenceComplete) {
